...
# 現在のGitHub Project番号。必要なら呼び出し時に上書きできる。
PROJECT_NUMBER ?= 1
...
help:
	@printf '%s\n' \
	  ...
	  '  make github-check [PROJECT_NUMBER=1]' \
	  ...
	  '  make project-status ISSUE_NUMBER=123 STATUS="In Progress" CONFIRM=1 [PROJECT_NUMBER=1]' \
	  '' \
	  'CONFIRM=1 がない場合、外部変更は実行しません。'

github-check:
	@./scripts/github.sh check-config "$(PROJECT_NUMBER)"
...
project-status:
	@./scripts/github.sh project-status "$(ISSUE_NUMBER)" "$(STATUS)" "$(CONFIRM)" "$(PROJECT_NUMBER)"
